...
- V3.2（已发布）：Dynamic Risk Budget + Portfolio Optimization（动态风险预算：factors/beta.py 个股 Beta、portfolio/risk_parity.py Risk Parity、portfolio/volatility_target.py 波动率目标、portfolio/industry_budget.py 行业风险预算、portfolio/beta_control.py Beta 控制、portfolio/dynamic_cash.py 动态现金、risk/risk_state.py 风险状态机、risk/drawdown_control.py 回撤仓位控制、portfolio/optimizer.py PortfolioOptimizerV32 十条流水线、portfolio/portfolio_manager.py PortfolioManagerV32、main_v32.py 演示）
- V3.3（已发布）：真实组合优化引擎 + AI Portfolio Rebalancer（data/historical.py AkShare 真实历史行情、factors/returns.py 收益率、portfolio/covariance.py 协方差、portfolio/transaction_cost.py 佣金/印花税/滑点、portfolio/target_portfolio.py 目标仓位、portfolio/position_sizer.py 100 股整数手、portfolio/rebalancer.py 调仓订单、portfolio/rebalance_decision.py BUY/SELL/HOLD 决策、portfolio/portfolio_manager.py PortfolioManagerV33 目标组合 + 成本感知调仓、main_v33.py 演示）
- V3.4（已发布）：机构级 Ensemble Portfolio Optimizer（optimization/mean_variance.py Mean-Variance、optimization/risk_parity.py Risk Parity、optimization/black_litterman.py Black-Litterman、optimization/ensemble.py Ensemble 融合、optimization/objective.py 目标函数、risk/constraints.py 单票/行业/Beta/最低现金约束、portfolio/liquidity.py 流动性约束、portfolio/turnover.py 换手率控制、portfolio/optimizer.py InstitutionalPortfolioOptimizer、portfolio/institutional_manager.py 机构级组合引擎、main_v34.py 演示）
- V3.5（已发布）：AI Portfolio Backtest & Stress Test Engine（backtest/portfolio_backtest.py PortfolioBacktest 逐日调仓回测（佣金/印花税/滑点/换手）、backtest/benchmark.py BenchmarkEngine 基准净值、backtest/walk_forward.py WalkForwardEngine 滚动前推、backtest/stress_test.py StressTestEngine 压力测试（crash_20/crash_30/high_volatility）、metrics/performance.py PerformanceMetrics 全套绩效指标、metrics/comparison.py PortfolioComparison 组合对比、metrics/robustness.py 策略可信度评分（0-100 分级）、validation/train_test.py TimeSeriesSplit 时间序列切分、validation/oos.py OOSValidator 样本外验证、main_v35.py 演示；先验证系统结构再接真实数据，禁止自动实盘）
- V3.6（已发布）：真实 A 股历史数据研究平台（data/akshare_client.py AkShareClientV36 带本地 CSV 缓存、data/price_loader.py PriceLoader 标准化历史数据、data/calendar.py TradingCalendar 交易日历、market/limit_rules.py LimitRule 涨跌停（主板10%/创业板科创板20%/北交所30%、0.002 容差）、market/suspension.py SuspensionDetector 停牌检测、market/trading_rules.py AShareTradingRules T+1 约束、backtest/costs.py TradingCostModel 佣金最低5元/印花税/滑点、backtest/execution.py ExecutionEngine 交易执行器（组合优化与执行彻底分开、100 股整数手、涨停不能买/跌停不能卖）、validation/data_quality.py DataQualityChecker 数据质量检查、main_v36.py 真实数据演示；V3.6 先提供接口，生产版接入中国交易所实际交易日历；LLM 永远不能直接“猜”行情）
- V3.7（已发布）：Institutional Data Integrity & Bias Control（Point-in-Time 数据与反偏差系统——data/point_in_time.py PointInTimeStore 时点数据（PITRecord：period_end→publish_date→available_date，2024-03-01 看不到 2024-03-28 才公布的财报）、data/universe.py HistoricalUniverse 历史股票池（IPO/退市生命周期）、data/corporate_actions.py CorporateActionStore 公司行为（分红/拆股）、data/availability.py AvailabilityChecker 数据可用日、validation/lookahead.py LookaheadDetector 前视偏差扫描、validation/leakage.py LeakageDetector 特征-标签泄漏、validation/temporal.py TemporalValidator 时间顺序校验、validation/survivorship.py SurvivorshipDetector 幸存者偏差、validation/audit.py BacktestAudit 统一审计、backtest/dataset.py CleanBacktestDataset 干净数据集、main_v37.py 演示）
- V3.8（已发布）：AI Alpha Research Engine（AI 因子研究引擎——factors/ 因子工厂：Value（PE/PB/PS 倒数）/ Momentum（20/60/120）/ Quality（ROE/ROIC/营收/利润增速）/ Volatility / Liquidity、alpha/ 因子检验：InformationCoefficient Rank IC、ICIRCalculator、QuantileAnalyzer 分位回测、FactorDecayAnalyzer 因子衰减、FactorCorrelation 相关性、Orthogonalizer 残差正交化、AlphaScorer 评分（STRONG/VALID/WEAK/REJECT）、experiments/ 因子实验框架（FactorExperiment + ExperimentStore JSON 结果）、main_v38.py 演示（随机数据 seed 42，先验证机制再接真实数据））
- V3.8.1（规划）：用 AkShare 真实 A 股数据流替换随机数据（5000 只 A 股 → Historical Universe → PE/PB/ROE → Point-in-Time Filter → Factor Matrix → Robust Alpha Library）
- V3.9（当前）：AI Alpha Discovery Engine（AI 自动因子挖掘——alpha/ 表达式树：expression.py AlphaExpression（叶子/一元/二元节点 + complexity 复杂度 + to_string）、operators.py AlphaOperators 九种运算（ADD/SUB/MUL/DIV 除零置 NaN/NEG/ABS/LOG/RANK/ZSCORE）、generator.py AlphaGenerator 随机生成（13 个候选因子、90% 因子叶 + 10% 常数叶、二元/一元算子随机组合）、evaluator.py AlphaEvaluator 递归求值、complexity.py ComplexityPenalty 复杂度惩罚（过拟合防御）、deduplicator.py AlphaDeduplicator SHA-256 去重、search.py AlphaSearchEngine 搜索（生成→去重→IC/ICIR→复杂度惩罚→按 Adjusted Score 排序）、library.py AlphaLibrary 优秀 Alpha 持久化（JSON）；evolution/ 遗传进化：alpha_genome.py AlphaGenome 基因（fitness=OOS*0.5+IC*0.2+ICIR*0.2-Complexity*0.01）、alpha_mutation.py AlphaMutation 变异（25% 换因子/60% 子节点变异/算子替换）、alpha_crossover.py AlphaCrossover 交叉、alpha_evolution.py AlphaEvolution 精英保留 10% + 交叉变异补足种群；validation/alpha_oos.py AlphaOOSValidator 样本外验证（train/test IC、|OOS IC|≥0.02 且≥|Train IC|*0.30）；main_v39.py 演示（算法验证版：随机数据 seed 42 构造隐藏 Alpha=ROE*0.4+Momentum20*0.3-Volatility20*0.2，300 候选×10 代进化，验证搜索系统能从噪声中找回隐藏结构；真正管线 30,000 Alpha→IC→ICIR→复杂度→相关性→OOS→Walk-Forward→Alpha Library；与 V2.8 双层进化融合——Alpha 进化（因子/公式）+ 策略进化（参数/规则））
